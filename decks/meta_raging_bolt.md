# raging_bolt — TCGplayer, September 2026

<!-- field-results -->
> ### Field results — 2026-09-11
> **62.4% mean · 62.0% median · 38 of 43 winning matchups · rank 5 of 44**
>
> Best `meta_ns_zoroark` 88% · worst `arbok_muk_trolley_darkbell` 40%.
>
> Full round robin, 200 games per pairing, every deck against every other — see [FIELD_RESULTS.md](FIELD_RESULTS.md). **Any win rate written in the body below this box was measured on an older engine and is not comparable** — not with this number and not with each other.
<!-- field-results -->


**Not my list.** From Natalie Millar's *The Best Pokémon TCG Decks Right
Now (September 2026)* on TCGplayer (2026-08-25), reproduced card-for-card.

The published lists give collector numbers without set codes, so every
line was resolved by number against this repo's pool. Basic Energy is
written without a set code, as everywhere else here.

Per the article a misnomer: the main attacker is usually `Mega Kangaskhan ex`, whose `Run Errand` is the draw engine, with `Raging Bolt ex`'s `Bellowing Thunder` as a once-a-game finisher. Four `Area Zero Underdepths` raise the Bench to 8.

## Decklist

```
Pokémon: 19
3 Meowth ex POR 62
3 Teal Mask Ogerpon ex TWM 25
3 Mega Kangaskhan ex MEG 104
2 Latias ex SSP 76
2 Raging Bolt ex TEF 123
1 Chien-Pao SSP 56
1 Fezandipiti ex ASC 142
1 Iron Leaves ex TEF 25
1 Lillie's Clefairy ex ASC 76
1 Passimian SSP 111
1 Wellspring Mask Ogerpon ex TWM 64

Trainer: 26
4 Area Zero Underdepths SCR 131
4 Crispin SCR 133
4 Energy Switch MEG 115
4 Ultra Ball MEG 131
3 Boss's Orders MEG 114
2 Night Stretcher ASC 196
2 Cyrano SSP 170
1 Ciphermaniac's Codebreaking TEF 145
1 Unfair Stamp TWM 165
1 Glass Trumpet ASC 189

Energy: 15
8 Basic Grass Energy
2 Basic Lightning Energy
2 Basic Fighting Energy
2 Basic Psychic Energy
1 Basic Water Energy

Total Cards: 60
```
